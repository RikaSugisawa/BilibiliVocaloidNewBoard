...
def ThirtyDaysAgoInt():  # 取得30天前时间戳
    return int(time.mktime((datetime.datetime.now() - datetime.timedelta(days=3000)).timetuple()))
...
nd=open('.\\'+datetime.datetime.now().strftime('%Y-%m-%d')+'.csv','w+',encoding='gbk')
...
if __name__ == '__main__':
    I=0
    for q in range(9, 50):  # 遍历50页搜索结果
        JUDGE = 0
        pg = json.loads(gethtml(q))  # 取得搜索结果
        html = pg['html']
        buf = 0  # 在html中当前搜索位置置零
        while (html.find('http://www.bilibili.com/video/av', buf + 1) > 0):  # 如果还能找得到视频(请跳至133行)
            print('buf=',buf)
            if (JUDGE > 0):  # 若完成搜索
               # RS = sorted(RS, key=lambda RS: -RS[3], )  # 按单位得分排序 ########################调试去掉排序
                for k in range(0, RS.__len__() - 1):  # 挨个视频
                    for j in range(0, 12):  # 挨个数据
                        if (j != 3):  # 若不是得分项
                            try:  # 试图写入文件
                                ...
                                print('Data not written.',RS[k][0])
                        else:  # 若是得分项
                            nd.write(str(int(RS[k][j])) + ',')  # 转成string再写
                    nd.write('\n')  # 写换行
