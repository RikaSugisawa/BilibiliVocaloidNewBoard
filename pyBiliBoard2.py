...
def req(url):#携带UA请求文本
    return urllib.request.urlopen(urllib.request.Request(url,None,{'User-Agent':'Mozilla/5.0 (Windows NT 6.3; rv:36.0) Gecko/20100101 Firefox/36.04'})).read().decode()
def gethtml(page):#请求搜索结果
    return req('http://search.bilibili.com/ajax_api/video?keyword=VOCALOID%E4%B8%AD%E6%96%87%E5%8E%9F%E5%88%9B%E6%9B%B2&order=pubdate&page='+str(page)+'&_=1475649909891')
...
def getss(nr):#获取视频数据
    ...
    js=json.loads(req('http://api.bilibili.com/archive_stat/stat?callback=jQuery17202070728806148786_1475647238270&aid='+nr+'&type=text&_=1475647238378'))
    view=js['data']['view']#获取播放、收藏、硬币、弹幕数据
    favorite=js['data']['favorite']
    coin=js['data']['coin']
    danmaku=js['data']['danmaku']
    js=json.loads(req('http://api.bilibili.com/x/reply?callback=jQuery172021295901065111744_1475652382382&jsonp=text&type=1&sort=2&oid='+nr+'&pn=1&nohot=1&_=1475652382486'))
    ...
    if(mTimeStamp<ThirtyDaysAgoInt()):#如果超过30天就中止
        ...
        scPlay=((view if view<=10000 else view*0.5+5000)*rpB*0.1 if rpB<10 else (view if view<=10000 else view*0.5+5000))#播放得点
        totalSc=scPlay+(25*comment+danmaku)*rpA+favorite*rpB#总得分
        daySc=totalSc/(GetNowTime()-mTimeStamp)*86400#单位得分
        RS.append((nr,title,up,daySc,str(int(totalSc)),str(view),str(danmaku),str(comment),str(favorite),str(coin),mtime))#结果写入list
        print('OK:',nr)#返回成功
    return 0
#void main()
for i in range(1,50):#遍历50页搜索结果
    ...
    while(html.find('http://www.bilibili.com/video/av',buf+1)>0):#如果还能找得到视频(请跳至94行)
        if(JUDGE>0):#若完成搜索
            RS=sorted(RS,key=lambda RS:-RS[3],)#按单位得分排序
            for i in range(0,RS.__len__()-1):#挨个视频
                for j in range(0,11):#挨个数据
                    if (j!=3):#若不是得分项
                        try:#试图写入文件
                            nd.write(RS[i][j]+',')
                        except:#写不进去就空着
